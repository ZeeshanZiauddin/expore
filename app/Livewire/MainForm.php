...
class MainForm extends Component implements HasForms
{
            ...
            ->schema(components: [
                ...
                Section::make()->schema([
                    Select::make('from')
                        ->options(
                            Airport::query()
                                ->select(['id', 'city', 'code', 'country']) // Fetch required columns

                                ->get()
                                ->groupBy('country')
                                ->mapWithKeys(fn($group, $country) => [
                                    $country => $group->mapWithKeys(fn($airport) => [
                                        $airport->id => "{$airport->city} ({$airport->code})",
                                    ])->toArray(),
                                ])
                                ->toArray()
                        )
                    ...
                    DatePicker::make('return_date')
                        ->label('Return')
                        ->prefixIcon('heroicon-s-calendar-date-range')
                        ->prefixIconColor(fn($get) => $get('status') === 'oneway' ? 'gray' : 'warning')
                        ->minDate(today())
    ...
    public function submit(): void
    {
        ...
        $booking = Query::create(
            [
                ...
                'email' => $data['email'],
                'phone' => $data['phone'],
                'passengers' => $data['details']['passenger_count'],
                'flight_class' => $data['details']['flight_class'],
            ]
        );

        $this->reset('data');

        $this->redirect(route('thanks', $booking->toArray()));
    }
    ...
    {
        return new HtmlString('<svg class="w-6 h-6 " fill="#ef4444" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="currentColor">
